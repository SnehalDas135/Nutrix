...
import { addMeal, getMeals, getUserId } from '../../lib/session.js';
...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    return res.status(200).json({ meals: getMeals(req) });
  }
  ...
  try {
    ...
    return res.status(201).json({ meal: addMeal(req, res, meal) });
  } catch (err) {
    return sendError(res, 500, 'Failed to save meal cookie', err.message);
  }
}
